...
public class CasaSubastas
{
    ...
    private ArrayList<Subasta> cargaDatos()
    {
        ...
        try
        {
            ...
            Subasta s=new Subasta("Adolfo","ordenador",100, 2019,0,1,12,0);
            s.pujar(new Puja("Luis",110));
            s.pujar(new Puja("Mario",120));
            ret.add(s);

            s=new Subasta("Adolfo","teclado",20, 2019,0,1,12,0);
            s.pujar(new Puja("Luis",23.20));
            s.setFechaFin(2016,8,1,12,0);
            ret.add(s);
            
            ret.add(new Subasta("Antonio","camiseta",20, 2018,10,5,15,30));
            
            s=new Subasta("Ricardo","cargador",15, 2018,10,18,8,0);
            ...
            s=new Subasta("Ricardo","smartphone",200, 2019,0,1,12,0);
            s.pujar(new Puja("Luis",215));
            s.pujar(new Puja("Pablo",250));
            s.setFechaFin(2016,7,28,12,0);
            ret.add(s);
            
            s=new Subasta("Mario","ipad",325, 2018,6,1,20,15);
            ret.add(s);
            
            s=new Subasta("Adolfo","monitor",65, 2018,9,14,12,0);
    ...
    {
        for (int i=0;i<subastasPublicadas.size();i++){
            if(!subastasPublicadas.get(i).estaCerrada()){
                subastasPublicadas.get(i).cerrar();
            }
     ...
     * SubastaIncorrecta
     */
    public void pujar(int idSubasta, Puja puja) throws SubastaIncorrecta, PujaInvalida
    {
        //A hacer por el alumno
        Subasta sub = null;
        ...
        for(int i=0;i<subastasPublicadas.size();i++){
            ...
            if(subastasPublicadas.get(i).getId() == idSubasta){
                sub = subastasPublicadas.get(i);
                if(!sub.getUsuario().equals(puja.usuario()) && !sub.estaCerrada()){
                    double valorPuja;
                    if(sub.pujaLider() == null){
                        valorPuja = sub.getPrecioSalida();
                    }else{
                        valorPuja = sub.pujaLider().precioOferta();
                    }
                    if(valorPuja < puja.precioOferta()){
                        sub.pujar(puja);
                        todoOk = true;
                    }else{
                        throw new PujaInvalida("Precio incorrecto");
                    }
                }else{
                    throw new PujaInvalida("Problema con la subasta");
                }
            }
        }
        if(!todoOk){
            throw new SubastaIncorrecta("Subasta no encontrada");
        }
    }
     ...
     * fichero un resumen economico con el n�mero de subastas
     * cerradas, el n�mero de subastas desiertas (sin pujas) y el
     * total recaudado
     */
